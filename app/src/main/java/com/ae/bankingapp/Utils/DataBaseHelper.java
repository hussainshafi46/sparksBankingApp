package com.ae.bankingapp.Utils;
...
import com.ae.bankingapp.Models.Transfers;
...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    private void insertToTransfers(Transfers transfer) {
        /*
    ...
    public List<Transfers> getMyTransfers(long account_no) {
        /*
        ...
        */
        ...
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            transferList.add(new Transfers(cursor.getString(1),cursor.getLong(2), cursor.getLong(3), cursor.getDouble(4)));
            cursor.moveToNext();
        }
        database.close();
        return transferList;
    ...
    public boolean sendMoney(long from, long to, double amount) {
        if (changeAmount(String.valueOf(from), -amount) && changeAmount(String.valueOf(to), amount)) {
            insertToTransfers(new Transfers(null, from,to,amount));
            return true;
        }
        return false;
    }
